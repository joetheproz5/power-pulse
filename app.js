...
function applyLanguage(next){language=next;localStorage.setItem("power-language",next);document.documentElement.lang=next;document.documentElement.dir=next==="ar"?"rtl":"ltr";document.querySelectorAll("[data-i18n]").forEach(el=>el.textContent=t(el.dataset.i18n));document.querySelectorAll("[data-i18n-aria]").forEach(el=>el.setAttribute("aria-label",t(el.dataset.i18nAria)));document.querySelectorAll(".language-option").forEach(el=>el.classList.toggle("is-selected",el.dataset.language===next));sendNotificationPreference(notificationsEnabled());updateNotificationUI();if(currentData)render(currentData)}
