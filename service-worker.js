const CACHE = "hart-el-sett-power-v48";
const APP_FILES = ["./", "./index.html", "./styles.css?v=20260910.35", "./app.js?v=20260911.29", "./manifest.webmanifest", "./icon.svg"];
...
async function showPowerNotification(title,body){await self.registration.showNotification(title,{body,icon:"icon.svg",badge:"icon.svg",tag:"hart-el-sett-power",renotify:true,data:{url:self.registration.scope}});if("setAppBadge"in navigator)await navigator.setAppBadge(1)}
...
self.addEventListener("notificationclick",event=>{event.notification.close();const target=new URL(event.notification.data?.url||self.registration.scope,self.registration.scope);event.waitUntil(Promise.all([clients.matchAll({type:"window",includeUncontrolled:true}).then(windows=>{const existing=windows.find(client=>client.url.startsWith(self.registration.scope));return existing?existing.focus():clients.openWindow(target.href)}),"clearAppBadge"in navigator?navigator.clearAppBadge():Promise.resolve()]))});
